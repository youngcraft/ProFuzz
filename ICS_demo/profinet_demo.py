...
class ProfinetPTCP(Packet):
	...
	fields_desc = [BitFieldLenField("Padding", 0, 96),  # 12 bytes padding
	               XShortField("SequenceID", None), XShortField("Padding2", 0),  # 2 bytes padding
	               XIntField("Delay1ns", None), XShortField("TLVHeader", None),
	               BitFieldLenField("PortMACAddress", None, 48), XShortField("TLVHeader2", None)]
...
class DCPIdentityRequest(Packet):
	...
	fields_desc = [XByteField("ServiceID", 5),  # 5=Identify
	               XByteField("ServiceType", None), XIntField("Xid", None), XShortField("ResponseDelay", None),
	               XShortField("DCPDataLength", None), XByteField("Option", None),  # 2=Device Properties
...
class writeRequest(Packet):
	...
	fields_desc = [ByteField("VLAN", 4), XByteField("Ethertype", 0x0800), ShortField("IPUDP", 28), ByteField("RPC", 80),
	               ByteField("NDR", 20), ByteField("WriteBlock", 64), ByteField("WriteData", None), ByteField("FCS", 4)]
...
class ProfinetCyclicRealTimeFrame(Packet):
	...
	fields_desc = [XShortField("FrameID", 0x0),  # 0x8000 (SPS) or 0x8061 (robot)
	               ...
	               ShortField("CycleCounter", 0), XByteField("DataStatus", 0x35),  # 0x35: Valid and Primary, OK and Run
...
class ProfinetAlarmFrame(Packet):
	...
	fields_desc = [XShortField("AlarmDstEndpoint", None), XShortField("AlarmSrcEndpoint", None),
	               XByteField("PDUType", None), XByteField("AddFlags", None), XShortField("SendSeqNum", None),
	               XShortField("AckSeqNum", None), XShortField("VarPartLen", None), XByteField("ErrorCode", None),
	               XByteField("ErrorDecode", None), XByteField("ErrorCode1", None), XByteField("ErrorCode2", None)]
